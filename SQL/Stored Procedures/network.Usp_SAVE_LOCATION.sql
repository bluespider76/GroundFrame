...
**
*******************************/
IF EXISTS (SELECT 1 FROM sys.procedures WHERE object_id = OBJECT_ID('network.Usp_SAVE_LOCATION'))
BEGIN
	DROP PROC [network].Usp_SAVE_LOCATION
END
GO

CREATE PROC [network].[Usp_SAVE_LOCATION]
(
	@itemno INT = 0,
	@ymdv INT,
	@location_itemno INT,
...
BEGIN
	...
	BEGIN
		SET @message = N'Error executing [network].[Usp_SAVE_LOCATION]:- You must provide a valid name in @name.';
	...
	BEGIN
		SET @message = N'Error executing [network].[Usp_SAVE_LOCATION]:- System Location and Parent Location cannot both be NULL. Please supply either @location_itemno or @parent_location_itemno.';
		RAISERROR (@message, 16 ,1);
		RETURN;
	END
	...
	BEGIN
		SET @message = N'Error executing [network].[Usp_SAVE_LOCATION]:- You are updating a parent location. Please supply @latitude.';
		RAISERROR (@message, 16 ,1);
		RETURN;
	END
	...
	BEGIN
		SET @message = N'Error executing [network].[Usp_SAVE_LOCATION]:- You are updating a parent location. Please supply @longitude.';
		RAISERROR (@message, 16 ,1);
		RETURN;
	END
